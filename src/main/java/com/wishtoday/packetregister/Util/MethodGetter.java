...
public class MethodGetter implements ReflectGetter<Method> {
    ...
    public Method loadAndGet(Class<?>... classes) {
        Class<?> clazz;
        try {
            clazz = Class.forName(this.getClassName());
            return clazz.getDeclaredMethod(this.methodName, classes);
            ...
            log.error("method {} {}doesn't exist", this.methodName, Arrays.toString(classes), e);
            return null;
